...
func _setup_ui() -> void:
	...
	main_vbox.custom_minimum_size = Vector2(680, 580)
	...
	preset_option.add_item("Deep Monster (Slow & Low Pitch)")
	preset_option.add_item("High Chipmunk (Fast & High Pitch)")
	preset_option.add_item("Robot Effect")
	...
	# --- Sliders (Playback Speed Controls) ---
	var speed_hbox = HBoxContainer.new()
	main_vbox.add_child(speed_hbox)

	var s_label = Label.new()
	s_label.text = "⏩ Playback Speed / Tempo (0.25x - 3.0x): "
	speed_hbox.add_child(s_label)
	...
	speed_slider.custom_minimum_size = Vector2(160, 20)
	speed_slider.value_changed.connect(func(val): speed_label.text = str(val) + "x")
...
func _on_preset_selected(index: int) -> void:
	match index:
		0:
			...
			speed_slider.value = 1.0
		1:
			pitch_slider.value = 0.65
			speed_slider.value = 0.8
		2:
			pitch_slider.value = 1.60
			speed_slider.value = 1.3
		3:
			pitch_slider.value = 1.20
			speed_slider.value = 1.0
...
func _on_play_pressed() -> void:
	if recording and audio_player:
		...
		# Combines Pitch Modulation and Speed Control Scale
		audio_player.pitch_scale = pitch_slider.value * speed_slider.value
		audio_player.volume_db = linear_to_db(volume_slider.value)
		audio_player.play()
		status_label.text = "Status: ▶️ Playing back audio stream at " + str(speed_slider.value) + "x Speed..."
